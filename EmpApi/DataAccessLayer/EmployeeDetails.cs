...
using EmpApi.CommonUtilities;

namespace EmpApi.DataAccessLayer
{
    public class EmployeeDetails
    {
        ...
        public ReturnObject GetDetailsByEmail(string Email)
        {
            ...
            SqlConnection DbCon = new SqlConnection(Con);
            DbCon.Open();  //Opening Connection for Database
            string GetEmailData = "spInfoByEmail";
            ...
            {
                returnObj.Message = (string)cmd.Parameters["@ResponseMessage"].Value;
        ...
        public ReturnMessage PostDataLogic(PostEmployee InsertEmp)
        {
        ...
        public ReturnObject CredentialCheckLogic(LoginCheck login)
        {
            var returnObj = new ReturnObject();
            ...
            SqlConnection DbCon = new SqlConnection(Con);
            DbCon.Open();  //Opening Connection for Database
            string Check = "spCredentialsCheck";
            SqlCommand cmd = new SqlCommand(Check, DbCon)
            {
                CommandType = CommandType.StoredProcedure
            };
        ...
        public List<Employee> CheckEmailOrUsernameLogic(string Email, string Username)
        {
            ...
            SqlConnection DbCon = new SqlConnection(Con);
            DbCon.Open();  //Opening Connection for Database
            string CheckEmailorUsername = "spCheckEmailorUsername";
            SqlCommand cmd = new SqlCommand(CheckEmailorUsername, DbCon)
            {
                CommandType = CommandType.StoredProcedure
            };
            cmd.Parameters.AddWithValue("@Email", Email);
        ...
        public Ismail IsEmailLogic(string Email)
        {
            ...
            SqlConnection DbCon = new SqlConnection(Con);
            DbCon.Open();  //Opening Connection for Database
            string Mail = "spIsEmail";
            SqlCommand cmd = new SqlCommand(Mail, DbCon)
            {
                CommandType = CommandType.StoredProcedure
            };
            cmd.Parameters.AddWithValue("@Email",Email);
            ...
            if (res > 0 )
            {
                MailObj.IsEmail = true;
            }
            else
            {
                MailObj.IsEmail = false;
            }
